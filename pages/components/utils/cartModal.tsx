import { faCircleCheck, faXmark } from "@fortawesome/free-solid-svg-icons";
import { FontAwesomeIcon } from "@fortawesome/react-fontawesome";
import React from "react";
import styled from "styled-components";

const ModalContainer = styled.div`
  .modal-container {
    position: fixed;
    top: 0;
    right: 0;
    bottom: 0;
    left: 0;
    z-index: 1000;
    background-color: rgba(0, 0, 0, 0.5);
    display: flex;
    pointer-events: all;
    transition: opacity 250ms ease;
  }

  .modal {
    margin: auto;
    width: 70vw;
    height: 350px;
    background-color: white;
    border-radius: 0.2rem;
  }
`;

export default function CartModal({ showHandler }: any) {
  const modalcloseHandler = (e: any) => {
    if (e.target.className === "modal-container") {
      showHandler(false);
    }
  };

  return (
    <ModalContainer onClick={modalcloseHandler}>
      <div className="modal-container">
        <section className="modal">
          <header className="modal-header h-10 flex justify-end items-center">
            <button
              onClick={() => showHandler(false)}
              className="modal-close flex items-start"
            >
              <FontAwesomeIcon
                className="close text-xl hover:text-gray-500 pr-3"
                icon={faXmark}
              />
            </button>
          </header>
          <hr className="w-full" />
          <div
            className="modal-content"
            tw="padding[0px 10px] height[85%] flex pt-2"
          >
            <div className="left-section h-full" tw="width[65%] flex flex-col">
              <div
                className="headingLeftSection"
                tw="width[full] height[50px] font-size[18px] color[green] flex items-center"
              >
                <span tw="mr-1">
                  <FontAwesomeIcon icon={faCircleCheck} />
                </span>
                <span tw="margin-right[3px]">1</span> new Item(s) have been
                added to your cart
              </div>
              <div
                className="orderDetailContainer"
                tw="w-full height[150px] flex"
              >
                <div className="imageContainer" tw="width[30%] h-full">
                  <img
                    tw="h-full object-fill"
                    src="data:image/jpeg;base64,"
                    alt="asd"
                  />
                </div>
                <div
                  className="descriptionContainer"
                  tw="width[70%] h-full padding[10px 15px 5px 15px] flex flex-col justify-between"
                >
                  <div
                    className="description"
                    tw="max-height[94px] min-height[40px] height[auto] overflow[hidden]"
                  >
                    asdasd asdasd sadsda asdasd sadhb kjsdb ksdb kjs ljsdsa
                    jsasdaj
                  </div>
                  <div
                    className="priceContainer"
                    tw="height[40px] mt-2 flex justify-between items-center"
                  >
                    <span tw="color[#f57224]">Rs. 4323</span>
                    <div>
                      <span tw="opacity-50">Qty:</span> 1
                    </div>
                  </div>
                </div>
              </div>
              <hr />
              <div
                className="actionContainer"
                tw="w-full flex justify-center items-center height[100px]"
              >
                <button tw="width[43%] mr-4 border[1px solid #f57224] color[#f57224] hover:color[#c75816]  padding[6px 10px] border-radius[4px]">
                  CONTINUE SHOPPING
                </button>
                <button tw="width[35%] mr-6 background-color[#f57224] color[white] padding[8px 10px] border-radius[4px] hover:background-color[#df6720]">
                  GO TO CART
                </button>
              </div>
            </div>
            <div
              className="right-section"
              tw="width[35%] flex flex-col background-color[rgb(236 236 236 / 51%)] padding[10px 6px]"
            >
              <h2 tw="text-align[center]">
                My Shopping Cart <span tw="font-size[13px]">(22 items)</span>
              </h2>
            </div>
          </div>
        </section>
      </div>
    </ModalContainer>
  );
}
